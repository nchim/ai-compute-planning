...
**Status:** v0.2 — 2026-09-15. Updated to what is built (WS1–WS13). This doc + the proto contract are
the source of truth. Read `product-brief.md` for the product framing and
...
state `{plan, result, selection, proposals, history, baseline, compare, error}` with a serializable
command log. Three drivers emit the same commands, so behavior is identical however it is triggered:
...
Commands as built: `loadPlan` · `setField` · `applyPatch` · `proposeChange` / `acceptProposal` /
`rejectProposal` · `undo` / `redo` · `setBaseline` / `clearBaseline` / `toggleCompare` · `select` ·
`resultReceived` · `errorRaised` / `clearError`. A rejected command (bad path, no baseline, …) is logged
with its reason and raises a visible error rather than mutating state. **Optimize is an action, not a
command:** `store.optimize()` runs the optimizer on a clone with `phasing.mode=OPTIMIZE`, stores the reply
under the same stale-reply guard as analyze and leaves the live plan untouched; "Apply best plan" and the
Copilot's `run_optimize` both go through it. A mutation → re-run `Analyze` → re-render is the same
cycle for all three drivers, and an analyze in flight when a newer mutation is scheduled is discarded.
...
- The SPA (dev/harness builds, `VITE_HARNESS=1`) exposes **`window.__harness`** over the bus
  (`web/src/harness/api.ts`): `loadPlan`, `getPlan`, `getResult`, `setControl`, `listControls`,
  ...
  `getConsoleErrors`. The WS10/WS12 branches add `optimize`, `proposeChange`, `getCopilotSnapshot`
  and `loadFixture`.
...
Cloud Run terminates TLS; `--allow-unauthenticated` is deliberate (our Basic Auth gates instead).
...
4. For "optimize this," the Copilot patches `optimization.*` / `phasing.policy`, calls `run_optimize`
   (→ `store.optimize()`), interprets the frontier, and proposes the best plan as an accept/undo card.
